...
bool _flw_isVisible(uint instanceIndex, uint modelIndex) {
    ...
    // HiZ occlusion test. Requires vanilla Framebuffer.depthBuffer to be a sampleable depth
    // texture, which MixinFramebuffer guarantees whenever this shader runs (mixin and
    // shader ship in the same flywheel jar).
    if (isVisible) {
