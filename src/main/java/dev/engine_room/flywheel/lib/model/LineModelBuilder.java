...
public final class LineModelBuilder {
    ...
    public LineModelBuilder line(float x1, float y1, float z1, float x2, float y2, float z2) {
        ...
        // Normal encodes the line's orientation; the vertex shader uses it to expand the
        // line into a screen-space quad of correct width.
        float dx = x2 - x1;
        float dy = y2 - y1;
        float dz = z2 - z1;
    ...
    public Model build() {
        if (vertexCount == 0 || data == null || vertexView == null) {
        ...
        boundingSphere.w += 0.1f;  // pad for line width
    ...
    private static final class LineMesh implements Mesh {
        // Each "line segment" expanded into 4 vertices → 2 triangles (6 indices) in (3*count/2) total.
        ...
        LineMesh(VertexList vertexList, Vector4fc boundingSphere) {
