...
public class LightUpdatedVisualStorage {
    ...
    private void addInner(LightUpdatedVisual visual, SectionTracker tracker) {
        if (tracker.sections().isEmpty()) {
            ...
            visual2Sections.put(visual, LongSets.unmodifiable(new LongArraySet(0)));
    ...
    /**
     * Cheap test for "is at least one updater registered for this section." Matches the gate
     * the {@link #plan} loop uses to decide between dispatching a task and decrementing the
     * outer synchronizer. May return true for a section whose last visual was just removed
     * (the updater list lingers in the map until the next add); callers that only need to
     * avoid the manager-fanout overhead can treat that as a benign false positive.
     */
    public boolean hasUpdatersIn(long section) {
        return section2Updaters.containsKey(section);
    }
