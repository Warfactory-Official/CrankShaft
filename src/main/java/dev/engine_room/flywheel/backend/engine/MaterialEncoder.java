...
public final class MaterialEncoder {
    ...
    private static final int DEPTH_TEST_LENGTH = MoreMath.ceillog2(DepthTest.values().length);
    private static final int TRANSPARENCY_LENGTH = MoreMath.ceillog2(Transparency.values().length);
    private static final int WRITE_MASK_LENGTH = MoreMath.ceillog2(WriteMask.values().length);
    private static final int USE_OVERLAY_LENGTH = 1;
    private static final int USE_LIGHT_LENGTH = 1;
    private static final int CARDINAL_LIGHTING_MODE_LENGTH = MoreMath.ceillog2(CardinalLightingMode.values().length);
