...
public final class InstanceTree {
    ...
    private InstanceTree(ModelTree source, @Nullable TransformedInstance instance, InstanceTree[] children) {
        ...
        // Port's TransformedInstance is slab-in-place: it has no aliased `pose` matrix field to mutate.
    ...
    public void loadPose(PartPose pose) {
        // 26.2 PartPose is a 9-float record carrying scale; read it (matching vanilla ModelPart.loadPose)
        // instead of upstream's hard-coded DEFAULT_SCALE, so 26.2 can bake scale into an initialPose.
        x = pose.x();
        y = pose.y();
        z = pose.z();
        xRot = pose.xRot();
