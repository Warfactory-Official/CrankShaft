 ...
 *
 * <br> Implementing one or more of these will give an {@link AbstractEntityVisual}
 * access to more interesting and regular points within a tick or a frame.
 *
...
public abstract class AbstractEntityVisual<T extends Entity> extends AbstractVisual implements EntityVisual<T> {
    ...
    /**
     * Port: vanilla's first-person camera-entity suppression in {@code LevelExtractor#extractVisibleEntities};
     * visualized entities bypass that loop, so a ridden/spectated visual must re-apply the test.
     */
    protected boolean isFirstPersonCameraEntity() {
        Camera camera = Minecraft.getInstance().gameRenderer.mainCamera();
        return camera.entity() == entity && !camera.isDetached()
