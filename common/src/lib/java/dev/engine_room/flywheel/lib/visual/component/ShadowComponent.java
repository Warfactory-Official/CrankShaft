 ...
 * <br>
 * The shadow will be cast on blocks at most {@code min(radius, 2 * strength - 1)} blocks below the entity.</p>
 */
public final class ShadowComponent implements EntityComponent {
    ...
    private void setupInstances(DynamicVisual.Context context) {
        ...
        // 26.2: EntityRenderer.extractShadow's depth = pow / 0.5 - 1 (upstream/1.21.1 vanilla used strength * 2),
        // shrinking the downward cast as the strength fades; negative skips the y loop entirely.
        float castDistance = Math.min(strength * 2 - 1, radius);
    ...
    private void setupInstance(ChunkAccess chunk, double entityX, double entityZ, float strength) {
        // TODO: cache this?
        ...
        // Subtract the origin in double before casting (like the body's getVisualPosition); casting the full
        // world coord first drifts the shadow off the entity by pixels at large coordinates.
        instances.get()
                 .write((float) minX, (float) minY, (float) minZ,
                         (float) (entityX - renderOrigin.getX()), (float) (entityZ - renderOrigin.getZ()),
                         (float) (maxX - minX), (float) (maxZ - minZ), alpha, this.radius)
                 .setChanged();
    }
