...
uint _flw_fetchSolid3x3x3(uint sectionOffset, ivec3 blockInSectionPos) {
    uint ret = 0u;
    // 26.2: fetch per (y, z) ROW -- a row's 3 x-neighbors are CONSECUTIVE bits, so two word fetches (one
    // when the row doesn't straddle a word) cover what 27 independent _flw_isSolid calls fetched. The
    // shifted runtime addressing defeats compiler CSE across the old unrolled macros, so the sharing must
    // be explicit. Bit i of ret keeps the exact fori y, z, x order (x-neighbors are consecutive bits).
    uint base = sectionOffset + _FLW_SOLID_START_INTS;
    uint i = 0u;
    for (int y = -1; y <= 1; y++) {
        for (int z = -1; z <= 1; z++) {
            uint rowBit = uint(int(blockInSectionPos.x) - 1
                    + (int(blockInSectionPos.z) + z) * 18
                    + (int(blockInSectionPos.y) + y) * 324);
            uint shift = rowBit & 31u;
            uint lo = _flw_indexLight(base + (rowBit >> 5u));
            uint hi = _flw_indexLight(base + ((rowBit + 2u) >> 5u));
            uint bits3 = shift == 0u ? lo : ((lo >> shift) | (hi << (32u - shift)));
            ret |= (bits3 & 7u) << i;
            i += 3u;
        }
    }
...
uint[27] _flw_fetchLight3x3x3(uint sectionOffset, ivec3 blockInSectionPos, uint solidMask) {
    ...
    // 26.2: the same row-word scheme as _flw_fetchSolid3x3x3 -- a row's 3 consecutive light BYTES come from
    // two word fetches (one when the row doesn't straddle) instead of 27 independent _flw_lightAt calls.
    // lights[] keeps the exact fori y, z, x order (a row's x-neighbors are consecutive indices).
    uint base = sectionOffset + _FLW_LIGHT_START_INTS;
    uint i = 0u;
    for (int y = -1; y <= 1; y++) {
        for (int z = -1; z <= 1; z++) {
            uint rowByte = uint(int(blockInSectionPos.x) - 1
                    + (int(blockInSectionPos.z) + z) * 18
                    + (int(blockInSectionPos.y) + y) * 324);
            uint shift = (rowByte & 3u) << 3u;
            uint lo = _flw_indexLight(base + (rowByte >> 2u));
            uint hi = _flw_indexLight(base + ((rowByte + 2u) >> 2u));
            uint bytes3 = shift == 0u ? lo : ((lo >> shift) | (hi << (32u - shift)));
            lights[i] = (bytes3 & 0xFu) | (((bytes3 >> 4u) & 0xFu) << 10) | (uint((solidMask & (1u << i)) == 0u) << 20);
            lights[i + 1u] = ((bytes3 >> 8u) & 0xFu) | (((bytes3 >> 12u) & 0xFu) << 10) | (uint((solidMask & (1u << (i + 1u))) == 0u) << 20);
            lights[i + 2u] = ((bytes3 >> 16u) & 0xFu) | (((bytes3 >> 20u) & 0xFu) << 10) | (uint((solidMask & (1u << (i + 2u))) == 0u) << 20);
            i += 3u;
        }
    }
