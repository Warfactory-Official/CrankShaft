...
vec4 flw_sampleNearest(sampler2D source, vec2 uv, vec2 pixelSize) {
    vec2 du = dFdx(uv);
    vec2 dv = dFdy(uv);
...
vec4 flw_sampleRGSS(sampler2D source, vec2 uv, vec2 pixelSize) {
    vec2 du = dFdx(uv);
    vec2 dv = dFdy(uv);
    vec2 texelScreenSize = sqrt(du * du + dv * dv);
    ...
    vec4 nearestColor = flw_sampleNearest(source, uv, pixelSize, du, dv, texelScreenSize);
...
vec4 flw_sampleAtlas(sampler2D source, vec2 uv) {
    ...
    return UseRgss == 1 ? flw_sampleRGSS(source, uv, pixelSize) : flw_sampleNearest(source, uv, pixelSize);
#endif
}
