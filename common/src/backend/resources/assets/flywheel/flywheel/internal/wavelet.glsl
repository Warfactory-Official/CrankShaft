...
float get_coefficients(in vec4 coefficients[4], int index) {
    return coefficients[index >> 2][index & 3];
}

float total_absorbance(in vec4 coefficients[4]) {
...
float absorbance(in vec4 coefficients[4], float depth) {
    float scale_coefficient = get_coefficients(coefficients, TRANSPARENCY_WAVELET_COEFFICIENT_COUNT - 1);
    if (scale_coefficient == 0) {
        return 0;
    }
...
float signal_corrected_absorbance(in vec4 coefficients[4], float depth, float signal) {
    float scale_coefficient = get_coefficients(coefficients, TRANSPARENCY_WAVELET_COEFFICIENT_COUNT - 1);
    if (scale_coefficient == 0) {
        return 0;
    }
...
float total_transmittance(in vec4 coefficients[4]) {
    return ABSORBANCE_TO_TRANSMITTANCE(total_absorbance(coefficients));
}

float transmittance(in vec4 coefficients[4], float depth) {
