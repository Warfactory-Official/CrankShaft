...
public final class MaterialEncoder {
    ...
    private static final int CARDINAL_LIGHTING_MODE_MASK = bitMask(CARDINAL_LIGHTING_MODE_LENGTH,
    ...
    public static int packUberShader(Material material) {
        // Upstream layout: fog index hi16 | cutout index lo16. The uber fragment (VK) dispatches
        // flw_fogFilter/flw_discardPredicate on these at runtime; compile-keyed pipelines ignore them.
        var fog = MaterialShaderIndices.fogIndex(material.fog());
        var cutout = MaterialShaderIndices.cutoutIndex(material.cutout());
        return (fog << 16) | (cutout & 0xFFFF);
    }
