...
public class SourceFile implements SourceComponent {
    ...
    private SourceFile(Identifier name, SourceLines source, ImmutableList<Import> imports, List<SourceFile> included,
                       String finalSource) {
        this.name = name;
        this.source = source;
    ...
    public static LoadResult empty(Identifier name) {
        return new LoadResult.Success(
                new SourceFile(name, new SourceLines(name, ""), ImmutableList.of(), ImmutableList.of(), ""));
    }

    public static LoadResult parse(Function<Identifier, LoadResult> sourceFinder, Identifier name,
                                   String stringSource) {
        ...
        for (Import i : imports) {
            ...
            try {
                ...
                failures.add(new Pair<>(fileSpan, new LoadError.MalformedInclude(e)));
                continue;
            }
            ...
            if (result instanceof LoadResult.Success s) {
                ...
                failures.add(new Pair<>(fileSpan, e.error()));
            }
        }
        if (!failures.isEmpty()) {
