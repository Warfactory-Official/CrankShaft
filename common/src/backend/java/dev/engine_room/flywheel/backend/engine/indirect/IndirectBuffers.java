...
public class IndirectBuffers {
    ...
    public final ResizableStorageArray visWords;
    public final ResizableStorageArray drawInstanceIndex2;
    public final ResizableStorageArray model2;
    public final ResizableStorageArray draw2;
    /**
    ...
    IndirectBuffers() {
        ...
        visWords = new ResizableStorageArray(INT_SIZE, INSTANCE_GROWTH_FACTOR);
        drawInstanceIndex2 = new ResizableStorageArray(INT_SIZE, INSTANCE_GROWTH_FACTOR);
        model2 = new ResizableStorageArray(MODEL_STRIDE, MODEL_GROWTH_FACTOR);
        draw2 = new ResizableStorageArray(DRAW_COMMAND_STRIDE, DRAW_GROWTH_FACTOR);
    }

    private static void bindRange(int binding, int handle, long bytes) {
        org.lwjgl.opengl.GL30.glBindBufferRange(GL_SHADER_STORAGE_BUFFER, binding, handle, 0, bytes);
    }
    ...
    void updateCounts(int instanceCount, int modelCount, int drawCount) {
        ...
        drawInstanceIndex2.ensureCapacity(instanceCount);
        model2.ensureCapacity(modelCount);
        draw2.ensureCapacity(drawCount);
        visWords.ensureCapacity(objectStorage.pageSlotCount() * 2L);
    ...
    public void bindForCull() {
        multiBind(0, 4);
        bindRange(6, visWords.handle(), visWords.byteCapacity());
    }

    public void bindForCullPass2() {
        multiBind(0, 2);
        bindRange(BufferBindings.DRAW_INSTANCE_INDEX, drawInstanceIndex2.handle(), drawInstanceIndex2.byteCapacity());
        bindRange(BufferBindings.MODEL, model2.handle(), model2.byteCapacity());
        bindRange(6, visWords.handle(), visWords.byteCapacity());
    }
    ...
    public void bindForApply2() {
        bindRange(BufferBindings.MODEL, model2.handle(), model2.byteCapacity());
        bindRange(BufferBindings.DRAW, draw2.handle(), draw2.byteCapacity());
    }
    ...
    public void bindForDraw2() {
        multiBind(1, 1);
        bindRange(BufferBindings.DRAW_INSTANCE_INDEX, drawInstanceIndex2.handle(), drawInstanceIndex2.byteCapacity());
        bindRange(BufferBindings.MODEL, model2.handle(), model2.byteCapacity());
        bindRange(BufferBindings.DRAW, draw2.handle(), draw2.byteCapacity());
        GlBufferType.DRAW_INDIRECT_BUFFER.bind(draw2.handle());
    }
    ...
    private void multiBind(int base, int count) {
        final long ptr = multiBindBlock.ptr();
        nglBindBuffersRange(GL_SHADER_STORAGE_BUFFER, base, count, ptr + base * INT_SIZE,
                ptr + OFFSET_OFFSET + base * PTR_SIZE, ptr + SIZE_OFFSET + base * PTR_SIZE);
    }
