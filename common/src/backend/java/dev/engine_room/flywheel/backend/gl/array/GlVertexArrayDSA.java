...
public class GlVertexArrayDSA extends GlVertexArray {
    ...
    private final int[] attributeBindings;
    ...
    public GlVertexArrayDSA() {
        attributeBindings = new int[MAX_ATTRIBS];
        Arrays.fill(attributeBindings, -1);
    ...
    private static boolean isSupported() {
        ...
        return GlCompat.ALLOW_DSA && Checks.checkFunctions(c.glCreateVertexArrays, c.glVertexArrayElementBuffer,
                c.glVertexArrayVertexBuffer, c.glVertexArrayBindingDivisor, c.glVertexArrayAttribBinding,
                c.glEnableVertexArrayAttrib, c.glVertexArrayAttribFormat, c.glVertexArrayAttribIFormat);
    }
    ...
    @Override
    public void bindAttributes(final int bindingIndex, final int startAttribIndex,
        ...
        for (var attribute : vertexAttributes) {
            ...
            if (!attribute.equals(attributes[attribIndex])) {
                if (attribute instanceof VertexAttribute.Float f) {
                    GL45C.glVertexArrayAttribFormat(handle, attribIndex, f.size(), f.type().glEnum, f.normalized(),
                            offset);
