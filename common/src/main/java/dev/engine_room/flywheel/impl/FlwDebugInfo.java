...
public final class FlwDebugInfo {
    ...
    private static void addInstancerDebugInfo(StringBuilder out,
    ...
    private static void addVisualizationManagerDebugInfo(@Nullable VisualizationManagerImpl manager,
                                                         StringBuilder out) {
        if (manager == null) {
            out.append('\n');
    ...
    private static void addImplDebugInfo(StringBuilder out) {
        ...
        // Port: getVersionStr() intentionally omitted; report the mod id instead.
        appendLine(out, "Flywheel: ").append(Flywheel.ID);
        appendLine(out, "Backend: ").append(BackendManagerImpl.getBackendString());
        appendLine(out, "Terrain: ").append(BackendConfig.INSTANCE.terrainMode().token());
    ...
    public static void addDebugInfo(Minecraft minecraft, List<String> systemInfo) {
        ...
        systemInfo.add("");
        systemInfo.add("Flywheel: " + Flywheel.ID);
        systemInfo.add("Backend: " + BackendManagerImpl.getBackendString());
        systemInfo.add("Terrain: " + BackendConfig.INSTANCE.terrainMode().token());
        ...
        systemInfo.add("Memory Usage: CPU: " + StringUtil.formatBytes(
                FlwMemoryTracker.getCpuMemory()) + ", GPU: " + StringUtil.formatBytes(FlwMemoryTracker.getGpuMemory()));
    }
}
