...
public class LayoutBuilderImpl implements LayoutBuilder {
    ...
    @Override
    public LayoutBuilder matrix(String name, FloatRepr repr, @Range(from = 2, to = 4) int rows,
    ...
    @Override
    public LayoutBuilder vectorArray(String name, ValueRepr repr, @Range(from = 2, to = 4) int size,
    ...
    @Override
    public LayoutBuilder matrixArray(String name, FloatRepr repr, @Range(from = 2, to = 4) int rows,
                                     @Range(from = 2, to = 4) int columns, @Range(from = 1, to = 256) int length) {
        return element(name, ArrayElementTypeImpl.create(MatrixElementTypeImpl.create(repr, rows, columns), length));
    }

    @Override
    public LayoutBuilder matrixArray(String name, FloatRepr repr, @Range(from = 2, to = 4) int size,
                                     @Range(from = 1, to = 256) int length) {
        ...
        for (int i = 0; i < elements.size(); i++) {
            ...
            if (GLSL_KEYWORDS.contains(name)) {
                throw new IllegalStateException(
                        "Element at index " + i + " has invalid name '" + name + "'; this is a GLSL keyword!");
            }
            ...
            for (int j = 0; j < name.length(); j++) {
                ...
                if (j == 0) {
                    if (!isLetter(c)) {
                        throw new IllegalStateException(
                                "Element at index " + i + " has invalid name '" + name + "'! Names must start with a letter.");
                    }
                } else {
            ...
            if (name.length() > Layout.MAX_ELEMENT_NAME_LENGTH) {
                throw new IllegalStateException(
                        "Element at index " + i + " has invalid name '" + name + "'! Names must not be longer than " + Layout.MAX_ELEMENT_NAME_LENGTH + " characters.");
            }

            int prevIndex = name2IndexMap.putIfAbsent(name, i);
            if (prevIndex != -1) {
                throw new IllegalStateException(
                        "Elements at indices " + prevIndex + " and " + i + " have the same name; this is not valid!");
            }
        }
